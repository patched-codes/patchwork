
/**
 * Calculates the sum of two numbers
 * @param {number} a - The first number
 * @param {number} b - The second number
 * @returns {number} The sum of the two numbers
 */
function a_plus_b(a, b) {
    return a + b;
}

/**
 * Executes a SQLite query on the specified database and invokes the provided callback for each result.
 * @param {Object} db - The SQLite database object to perform the query on.
 * @param {string} query - The SQL query to be executed.
 * @param {function} callback - The callback function to be invoked for each result returned by the query.
 * @returns {void} This function does not return anything.
 */
const sqlite = (db, query, callback) => {
    db.serialize(function () {
        db.each(query, callback);
    });
}

/**
 * Compares two objects based on a specified key in the objects.
 * @param {string}  keymap - The key to compare the objects on.
 * @param {Object} a - The first object to compare.
 * @param {Object} b - The second object to compare.
 * @returns {number} Returns -1 if a[keymap] < b[keymap], 1 if a[keymap] > b[keymap], and 0 if they are equal.
 */
const compare= function (keymap, a, b) {
    if (a[keymap] < b[keymap]) {
        return -1;
    } else if (a[keymap] > b[keymap]) {
        return 1;
    } else {
        return 0;
    }
}