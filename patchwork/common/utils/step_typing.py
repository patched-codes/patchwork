...
def validate_step_with_inputs(input_keys: Set[str], step: Type[Step]) -> Tuple[Set[str], Dict[str, str]]:
    ...
    allowed_modules = {"module1.typed", "module2.typed"}  # Add allowed modules here
    
    full_module_name = f"{module_path}.typed"
    
    if full_module_name not in allowed_modules:
        raise ValueError(f"Importing module '{full_module_name}' is not allowed")

    type_module = importlib.import_module(full_module_name)
