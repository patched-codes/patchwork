...
def validate_step_with_inputs(input_keys: Set[str], step: Type[Step]) -> Tuple[Set[str], Dict[str, str]]:
    authorized_modules = {"app.module1.typed", "app.module2.typed"}  # Whitelist of allowed modules

    module_path, _, _ = step.__module__.rpartition(".")
    step_name = step.__name__
    type_module_name = f"{module_path}.typed"

    if type_module_name not in authorized_modules:
        raise ValueError(f"Unauthorized module import: {type_module_name}")
