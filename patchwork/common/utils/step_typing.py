...
def validate_step_with_inputs(input_keys: Set[str], step: Type[Step]) -> Tuple[Set[str], Dict[str, str]]:
    allowed_modules = {"module1.typed", "module2.typed"}  # Add trusted module paths
    module_path, _, _ = step.__module__.rpartition(".")
    step_name = step.__name__
    typed_module_path = f"{module_path}.typed"
    if typed_module_path not in allowed_modules:
        raise ImportError(f"Module '{typed_module_path}' is not allowed.")
    type_module = importlib.import_module(typed_module_path)
