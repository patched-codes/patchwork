...
def validate_step_with_inputs(input_keys: Set[str], step: Type[Step]) -> Tuple[Set[str], Dict[str, str]]:
    ...
    if not hasattr(type_module, f"{step_name}Inputs") or not hasattr(type_module, f"{step_name}Outputs"):
        raise ValueError(f"Missing input or output model for step {step_name}")
