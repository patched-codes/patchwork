...
def validate_step_with_inputs(input_keys: Set[str], step: Type[Step]) -> Tuple[Set[str], Dict[str, str]]:
    ...
    # Whitelisted modules to be explicitly allowed
    allowed_modules = {"expected.module1.typed", "expected.module2.typed"} 

    if f"{module_path}.typed" not in allowed_modules:
        raise ValueError("Attempt to load an untrusted module")
