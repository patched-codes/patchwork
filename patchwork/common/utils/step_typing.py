...
def validate_step_with_inputs(input_keys: Set[str], step: Type[Step]) -> Tuple[Set[str], Dict[str, str]]:
    ...
    # Add a whitelist to prevent loading arbitrary code
    allowed_modules = ['mymodule.typed']  # Update with allowed module paths
    if module_path not in allowed_modules:
        raise ImportError("Unauthorized module access")
