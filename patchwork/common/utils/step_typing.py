...
def validate_step_with_inputs(input_keys: Set[str], step: Type[Step]) -> Tuple[Set[str], Dict[str, str]]:
    module_whitelist = {"allowed.module1", "allowed.module2"}  # Add the appropriate valid modules here.
    module_path, _, _ = step.__module__.rpartition(".")
    step_name = step.__name__
    if f"{module_path}.typed" not in module_whitelist:
        raise ValueError(f"Importing from {module_path}.typed is not allowed.")
