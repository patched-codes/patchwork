...
def validate_step_with_inputs(input_keys: Set[str], step: Type[Step]) -> Tuple[Set[str], Dict[str, str]]:
    ...
    allowed_modules = {
        "permitted_module1",
        "permitted_module2",
        "module_path.typed"  # This should be adjusted according to actual module paths that are allowed
    }
    
    # Check if the module_path.typed is in the whitelist
    if f"{module_path}.typed" not in allowed_modules:
        raise ValueError("Attempt to import a non-permitted module")
