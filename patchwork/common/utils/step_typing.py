...
def validate_step_with_inputs(input_keys: Set[str], step: Type[Step]) -> Tuple[Set[str], Dict[str, str]]:
    allowed_modules = {"module1.typed", "module2.typed"}  # Define the whitelist of allowed modules
    ...
    if module_to_import not in allowed_modules:
        raise ImportError(f"Importing module '{module_to_import}' is not allowed.")
