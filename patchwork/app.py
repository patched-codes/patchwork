...
def find_module(possible_module_paths: Iterable[str], patchflow: str) -> ModuleType | None:
    for module_path in possible_module_paths:
        ...
        try:
            valid_module_paths = ["allowed_module_1", "allowed_module_2"]  # Add more to the list if needed
            if module_path in valid_module_paths:
                return importlib.import_module(module_path)
            else:
                logger.error(f"Attempted to import untrusted module: {module_path}")
