...
def find_patchflow(possible_module_paths: Iterable[str], patchflow: str) -> Any | None:
    # Define a whitelist of allowed module paths for import
    allowed_module_paths = {"allowed_module1", "allowed_module2"}  # Update this set as needed
    ...
    for module_path in possible_module_paths:
        ...
        # Check if the module_path is in the whitelist before importing
        if module_path in allowed_module_paths:
            try:
            ...
            logger.debug(f"Module path {module_path} is not in the allowed whitelist")
