...
def find_patchflow(possible_module_paths: Iterable[str], patchflow: str) -> Any | None:
    trustable_modules = {"trusted_module1", "trusted_module2"}  # Add trusted modules here
    for module_path in possible_module_paths:
        ...
        try:
            if module_path in trustable_modules:  # Check if module path is in the trusted list
                module = importlib.import_module(module_path)
                logger.info(f"Patchflow {patchflow} loaded from {module_path}")
                return getattr(module, patchflow)
            else:
                logger.debug(f"Module path {module_path} not in the trusted list")
