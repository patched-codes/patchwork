...
def find_module(possible_module_paths: Iterable[str], patchflow: str) -> ModuleType | None:
    for module_path in possible_module_paths:
        ...
        if module_path in whitelist_modules:  # Assuming whitelist_modules is the list of allowed module names
            try:
                return importlib.import_module(module_path)
            except ModuleNotFoundError:
                logger.debug(f"Patchflow {patchflow} not found as a module in {module_path}")
        else:
            logger.debug(f"Importing {module_path} is not allowed due to whitelist restrictions")
