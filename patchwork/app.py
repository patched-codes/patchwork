...
def find_patchflow(possible_module_paths: Iterable[str], patchflow: str) -> Any | None:
    for module_path in possible_module_paths:
        ...
        try:
            module = importlib.import_module("safe_module")
            logger.info(f"Patchflow {patchflow} loaded from safe_module")
            return getattr(module, patchflow)
        except ModuleNotFoundError:
            logger.debug(f"Patchflow {patchflow} not found as a module in safe_module")
        except AttributeError:
            logger.debug(f"Patchflow {patchflow} not found in safe_module")
